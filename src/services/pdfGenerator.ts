...
export function generateBlueprintFormulaPdf(customFilename = 'crypto_review_lab_technical_whitepaper.pdf'): void {
  ...
  const pillars = [
    {
      ...
      detail: 'Weights are category-specific and deterministically fixed per protocol category (not one universal split), sourced from getCategoryDimensionWeights() in EvaluationBlueprint.ts, with absolute separation between evaluative utility scores and empirical security risk.'
    },
    {
  ...
  doc.setFontSize(6.8);
  doc.setTextColor(71, 85, 105);
  doc.text('Weights are category-specific and deterministically fixed per protocol category (not one universal split), sourced from getCategoryDimensionWeights() in EvaluationBlueprint.ts:', margin, y);
  y += 3.8;
  ...
  const avfModules = [
    ...
    { id: 'AVF-03', name: 'Methodology & Weighting Compliance', desc: 'Verifies that weights are category-specific and deterministically fixed per protocol category (not one universal split), sourced from getCategoryDimensionWeights() in EvaluationBlueprint.ts, and were strictly applied without ad-hoc parameter tampering.' },
