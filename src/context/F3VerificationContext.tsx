  ...
  const reviewedProjects = useMemo(() => {
    ...
    proOrders.forEach((order) => {
      ...
      const revObj: EnhancedReviewedProject = {
        ...
        category: base?.category || 'Web3 / Layer 1-2',
        overallScore: base?.overallScore ?? null,
        verdict: base?.verdict || 'Assessment in progress — verdict pending.',
        scores: base?.scores || undefined,
        riskLevel: base?.riskLevel || 'Pending',
        ...
        summary: base?.summary || `Assessment evaluation for ${order.projectName} (${order.projectSymbol}).`,
        isProOrder: true,
